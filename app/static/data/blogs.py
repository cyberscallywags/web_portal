...
def get_all_blog_data() -> list[dict]:
    blog_data = get_blog_data()
    return blog_data


def get_blog_data_by_slug(req_slug: str = "cyber-scallywags-community") -> list[dict]:
    blog_data = get_blog_data()
    return [blog for blog in blog_data if blog["slug"] == req_slug]


if __name__ == "__main__":
    # For testing purposes
    all_blogs = get_all_blog_data()
    assert 5 == len(all_blogs)

    specific_blog = get_blog_data_by_slug("python-code-nanny")
    assert 1 == len(specific_blog)
    assert specific_blog[0]["title"] == "Python Code Nanny"
